...
function button1(fname="recent-release" , y) {
  if (y === undefined) { name = fname; }
     

      fetch(`https://gogoanime.herokuapp.com/${name}`)
      .then((response) => response.json())
      .then(aniiem);
      function aniiem(data) {document.getElementById("content").innerHTML = `${data.map(aniem).join('')}`}
  
document.getElementById("next").addEventListener("click", function() {
      value = ++i;

      fetch(`https://gogoanime.herokuapp.com/${name}?page=${value}`)
      .then((response) => response.json())
      .then(aniiem);
      function aniiem(data) {document.getElementById("content").innerHTML = `${data.map(aniem).join('')}`}
